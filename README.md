...
## Гостевой доступ
- Сайт доступен без регистрации для чтения (каталог/страницы).
- Любые операции изменения данных (например, создание платежа, перерасчет доставки, возврат) требуют Bearer JWT и при отсутствии авторизации вернут `401` с подсказкой войти/зарегистрироваться.


## Окно логина/регистрации
- В `static/index.html` добавлено модальное окно с двумя формами: вход и регистрация.
- Поля в обеих формах: email, телефон, логин, пароль.
- Для MVP отправка форм демонстрационная (frontend alert), API можно подключить к `POST /api/auth/login` и `POST /api/auth/register`.

## Корзина в HTML версии
- Добавлено добавление товаров в корзину из карточек каталога.
- Добавлен просмотр корзины в секции `#cart` (позиции, количество, удаление, итоговая сумма).
- Состояние корзины хранится в `localStorage` (`avensio_cart`).
